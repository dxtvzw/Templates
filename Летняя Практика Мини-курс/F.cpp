    ...
    uint64_t q[], uint64_t u[], int ulen, const uint64_t d[], int dlen) noexcept
{
    ...
    for (int j = ulen - dlen - 1; j >= 0; --j)
    {
        ...
        uint64_t qhat;
        if (INTX_UNLIKELY((uint128{u1, u2}) == divisor))
        {
            qhat = ~uint64_t{0};
...
public:
    ...
    friend Modular operator-(const Modular& a, const Modular& b) {
        if (a.val < b.val) return a.val + mod - b.val;
        else return a.val - b.val;
    }
    friend Modular operator*(const Modular& a, const Modular& b) {
    ...
    friend void gcd(bigint a, bigint b, Modular& x, Modular& y) {
        if (a == 0) {
            x = Modular(0);
            y = Modular(1);
            return;
...
istream& operator>>(istream& istr, bigint& x) {
    string s;
    istr >> s;
    x = 0;
    for (char ch : s) {
...
vector<int> convert_to_p_system(bigint a, int p) {
    vector<int> res;
    while (a != 0) {
        res.push_back(int(a % p));
        a /= p;
    }
    return res;
}
...
istream& operator>>(istream& istr, Point& p) {
    ...
    if (a == "Z") {
        ...
        string b;
        istr >> b;
        p = Point(string_to_bigint(a), string_to_bigint(b));
        return istr;
    }
}
...
Point decode(const Point& x, const Point& y, const bigint& ord, const bigint& priv_key) {
    return binpow(x, ord - priv_key) * y;
}
...
void solve_f() {
    ...
    mod = p;
    bigint a = p - 3;
    string b_string("41058363725152142129326129780047268409114441015993725554835256314039467401291");
    bigint b = string_to_bigint(b_string);
    coef_a = a;
    coef_b = b;
    string g_x("48439561293906451759052585252797914202762949526041747995844080717082404635286");
    string g_y("36134250956749795798585127919587881956611106672985015071877198253568414405109");
    Point g(string_to_bigint(g_x), string_to_bigint(g_y));
    string ord_string("115792089210356248762697446949407573529996955224135760342422259061068512044369");
    bigint ord = string_to_bigint(ord_string);
    ...
    while (n--) {
        Point x, y;
        cin >> x >> y;
        Point res = decode(x, y, ord, priv_key);
        cout << vector_to_string(convert_to_p_system(res.x.val, 64)) << "\n";
    }
}
