...
Poly binpow(Poly a, bigint n, const Poly& mod) {
    Poly res(1);
    for (; n != 0; n /= 2) {
        if (n % 2 == 1) res = mult(res, a, mod);
        a = mult(a, a, mod);
    }
    return res;
}
...
Poly decode(const Poly& x, const Poly& y, bigint ord, const Poly& f, uint k) {
    return mult(y, binpow(x, ord - k, f), f);
}
...
void solve_d() {
    ...
    bigint ord = binpow(p, n) - 1;

    while (getline(cin, line)) {
        Poly x = read_poly(line);
        Poly y = read_poly();
        Poly cur = decode(x, y, ord, f, k);
